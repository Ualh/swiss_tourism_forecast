...
# Convert tourism_ts to tsibble and create models

# Fit Naive model
naive_fit <- tourism_ts %>% model(NAIVE = NAIVE(value))
naive_forecast <- naive_fit %>% forecast(h = 24)
naive_residuals <- residuals(naive_fit)
naive_metrics <- naive_fit %>% accuracy()

# Fit ETS model
ets_fit <- tourism_ts %>% model(ETS = ETS(value ~ error("A") + trend("A") + season("M")))
...
model_comparison <- data.frame(
  ...
  MAE = c(naive_metrics$MAE, ets_metrics$MAE, arima_metrics$MAE, arimax_metrics$MAE),
  MAPE = c(naive_metrics$MAPE, ets_metrics$MAPE, arima_metrics$MAPE, arimax_metrics$MAPE),
  RMSE = c(naive_metrics$RMSE, ets_metrics$RMSE, arima_metrics$RMSE, arimax_metrics$RMSE),
  AIC = c(naive_metrics$AIC, ets_metrics$AIC, arima_metrics$AIC, arimax_metrics$AIC),
  BIC = c(naive_metrics$BIC, ets_metrics$BIC, arima_metrics$BIC, arimax_metrics$BIC)
)
