...
# Convert to an interactive plotly object with specified width and height
interactive_plot <- ggplotly(p, width = 700, height = 300)
...
In this section we will focus on the total number of visitors in Vaud. We will use the ETS, ARIMA and TSRM models to forecast the number of visitors in Vaud over a 15-month horizon.
...
Automatically the model ARIMA(5,0,0)(0,1,1) was chosen. The graph shows that the seasonality observed in the historical data continues to manifest itself in the future forecasts, with recurring seasonal peaks and troughs.  Due to the drop observed in 2020, the trend is showing a slight downward trajectory, however, currently, there are no evident reasons for a decrease in the number of visitors.

To capture the positive trend we use model ARIMA (5,1,0)(0,1,1). Changing models explicitly using a differencing order of 1 in the non-seasonal component provide better flexibility in capturing any underlying trend patterns in the data. This adjustment allows the model to more effectively account for changes in the level of the time series data over time. The ARIMA(5,1,0)(0,1,1) model effectively captures both the positive trend and seasonality present in the data. The approach results in forecasts that closely align with observed patterns, leading to a narrow confidence interval enhancing the reliability of predictions for tourist arrivals in canton Vaud.

### TSLM 

In light of the significant drop in tourist numbers caused by the COVID-19 pandemic in 2019, it is crucial to address this anomaly effectively in our forecasting methodologies. The COVID-19 pandemic brought about a paradigm shift in tourist behavior, with travel restrictions and containment measures influencing tourist flows in complex ways. By incorporating the stringency index into our Time Series Linear Regression (TSLM) model, we can effectively capture the influence of these external factors on tourist trends. This approach allows us to recognize the dynamic nature of tourist behavior in response to changing policy environments.

The OxCGRT taken from [Our World In Data](https://datacatalog.med.nyu.edu/dataset/10394) provides data on government responses to COVID-19, including a Stringency Index that measures the severity of lockdown measures. The Stringency Index is calculated based on nine key metrics: School closures, Workplace closures, Cancellation of public events, Restrictions on public gatherings, Closures of public transport, Stay-at-home requirements, International travel controls, etc... Each metric is scored from 0 to 100, with higher scores indicating stricter responses. The overall Stringency Index is the average score of these metrics, reflecting the strictest sub-regional policies if they vary.

In our effort to forecast the number of tourists in Vaud for the next 15 months, we've utilized a Time Series Linear Regression (TSLM) model. This approach allows us to incorporate the stringency index for Switzerland into our forecasting framework effectively. Given our expectation of no restrictions for the next 15 months, we've set the stringency index to 0 for this period. This decision reflects our anticipation of a return to pre-pandemic levels of travel freedom and mobility. By incorporating this assumption into our forecasting model, we aim to provide a more optimistic outlook for tourist numbers in Vaud.
...
*see the appendix for a vizualisation of the 2 models*
...
*see the appendix for a vizualisation of the ARIMAX model*
...
![Models Metrics](images/zh_metrics.png){fig-align="center"}
